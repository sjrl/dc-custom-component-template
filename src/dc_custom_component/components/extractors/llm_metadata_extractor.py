...
class SebLLMMetadataExtractor:
    ...
    def __init__(  # pylint: disable=R0917
        ...
        :param pages: The page numbers to be used sent to the LLM.
        """
        self.prompt = prompt
    ...
    def _get_sub_document(self, document: Document) -> Document:
        """Return only the part of the document defined by the pages."""
        pages = document.content.split("\f")
        sub_pages = []
        for page in self.pages:
            page_idx = page - 1
            if page_idx >= len(pages):
                logger.warning(f"Document {document.id} has no content on page {page}.")
            else:
                sub_pages.append(pages[page_idx])
        sub_content = "\f".join(sub_pages)

        if sub_content == "":
            logger.warning(f"Document {document.id} has no content on the pages {self.pages}.")
        return Document(content=sub_content)
    ...
    def run(self, documents: List[Document]) -> Dict[str, Any]:
        ...
        """
        errors = {}
        # Don't send full doc just the parts defined by pages
        for document in documents:
            if self.pages:
                content = self._get_sub_document(document).content
            else:
